...
(`2fXkZ8QlmNbYcV1pR3sT`). Folders and dashboards use **snowflake-style integers
as strings** (`7494175832848465920`).
...
**Spelling preservation (role permissions, composite expressions).** `stream` and
`stream:_all_myorg` name the same grant; `{a} && {b}` and `({a} && {b})` are the
same expression. The server only reports one form. Whichever spelling was
configured is kept when the two describe the same thing.
...
`is_default` on templates, `token` on service accounts.
